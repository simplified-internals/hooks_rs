...
pub fn Controls(
    props: (Vec<Task>, Filter, Rc<dyn Fn(&dyn Fn(&Filter) -> Filter)>),
    ...
    row![
        ...
        .width(Fill),
        row![
            filter_button(("All", current_filter, Filter::All, set_filter.clone())),
            filter_button(("Active", current_filter, Filter::Active, set_filter.clone())),
...
fn filter_button(
    props: (
        ...
        Filter,
        Filter,
        Rc<dyn Fn(&dyn Fn(&Filter) -> Filter)>,
    ),
) -> Element<'static, Message> {
